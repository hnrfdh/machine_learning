...
output_dir = os.path.join("pertemuan5", "output")
os.makedirs(output_dir, exist_ok=True)

### 1 Load dataset pertemuan4 ###
base_dir = Path(__file__).resolve().parent.parent
dataset_path = base_dir / "pertemuan4" / "dataset" / "kelulusan_mahasiswa.csv"
...
### 5 Evaluasi Akhir pada Data Test ###
...
if hasattr(final_model, "predict_proba"):
    ...
    plt.tight_layout(); 
    roc_path = os.path.join(output_dir, "roc_test.png")
...
print("Model tersimpan ke {model_path}".format(model_path=model_path))

### 7 Endpoint Inference (Flask) ###
from flask import Flask, request, jsonify
import joblib, pandas as pd

app = Flask(__name__)
MODEL = joblib.load("{model_path}".format(model_path=model_path))
